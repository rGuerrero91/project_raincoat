...
export default function CategoryScreen({
  ...
  const [currentCroppedUrl, setCurrentCroppedUrl] = useState<
    string | undefined
  >(croppedImageUrl);
  ...
  useEffect(() => {
    if (detectedCategory) {
      console.log(
        "[CategoryScreen] Auto-selected detected category:",
        detectedCategory
      );
  ...
  useEffect(() => {
    ...
    const matchingDetection = allDetections.find(
      (det) => det.category === selectedCategory
    );

    if (matchingDetection) {
      const newCroppedUrl = yoloDetector.cropToBbox(
        originalImage,
      ...
      console.log(
        "[CategoryScreen] Updated crop preview for category:",
        selectedCategory
      );
          ...
          <div className="flex flex-col">
            ...
            <div className="grid grid-cols-2 gap-4">
              {categories.map((category) => {
                ...
                if (!IconComponent) {
                  console.error(
                    `Icon component is undefined for category: ${category.id}`
                  );
                  return null;
                }
                return (
